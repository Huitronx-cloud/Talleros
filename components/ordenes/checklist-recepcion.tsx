...
export default function ChecklistRecepcion({ ordenId, tallerId, onTerminar }: Props) {
  ...
  const handleFoto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFotoActual({ url: preview, file })
    setDescripcion('')
  }
  ...
  const handleGuardar = async () => {
    setGuardando(true)
    try {
      // Guardar fotos en fotos_diagnostico con tipo 'recepcion'
      if (fotos.length > 0) {
      ...
      // Guardar checklist como nota interna
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">
          Registra el estado del vehículo al recibirlo. Esto protege al taller y al cliente.
        </p>
      </div>
